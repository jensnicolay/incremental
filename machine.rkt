...
(define (ev e s g parent)
  (eval-path e '() s g parent))
  
(define (eval-path e field-path s g parent) ; general TODO: every fw movement should be restrained to previous path(s)
  (printf "ev ~v ~v in ~v\n" e field-path s)
  (match e
    ((«lit» _ d) ;(printf "-> ~v\n" ((lattice-α lat) d))
     d)
    ...
    ((«let» _ _ _ e-body)
     (let ((s* (state e-body (state-κ s))))
       (eval-path e-body field-path s* g parent)))
    ((«if» _ _ _ _)
     (let ((s* (successor s g)))
       (eval-path (state-e s*) '() s* g parent)))
    ((«set!» _ _ _)
     '<undefined>)
    ((«app» _ («id» _ x) e-rands) ;TODO: ((lam ...) rands)
     (let ((κ (state-κ s)))
       (let ((s* (successor s g)))
         (match s*
           (#f
            (let ((d-rands (map (lambda (e-rand) (eval-path e-rand field-path s g parent)) e-rands))
    ...
    ((«cons» _ e-car e-cdr)
     (match field-path
       ('() s)
       ((cons 'car field-path*) (eval-path e-car field-path* s))
       ((cons 'cdr field-path*) (eval-path e-cdr field-path* s))))
    ))
