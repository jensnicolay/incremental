...
(define (lookup-path-root x field-path s g parent)
  ...
  (define (follow-field-path field-path e s g parent)
      ...
      (match d
        ((state e* κ*)
          (match e*
            ...
            ((«car» _ («id» _ x))
             (lookup-path-root x (append field-path '(car)) s g parent))
            ((«cdr» _ («id» _ x))
             (lookup-path-root x (append field-path '(cdr)) s g parent))
            ((«cons» _ e-car e-cdr)
              (match field-path
                ((cons 'car '())
                (root e-car d))
    ...
    (if (null? field-path)
        (let ((var-root (lookup-var-root x s g parent)))
            (debug-print-out "#~v: lookup-path-root ~v ~v: ~a" (state->statei s) x field-path (user-print var-root))
              var-root)
        (let ((b (lookup-binding x s g parent)))                
          (match-let (((binding e-b κ-b) b))
                      ...
                      ((«set!» _ («id» _ (== x)) e-update)
                        (let ((b* (lookup-binding x s* g parent)))
                          (if (equal? b b*)
                              ...
                              (follow-field-path field-path e-update s* g parent)
                              (lookup-path-root-helper field-path s* g parent))))
